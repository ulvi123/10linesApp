...
def handle_start_command(robot:dict,params:dict):
...
def handle_park_command(robot:dict,params:dict)->dict:
    ...
    if robot.status != RobotStatus.IN_PROGRESS:
        raise HTTPException(syayus_code=status.HTTP_400_BAD_REQUEST,detail="Can only park a running robot")
    robot.status = RobotStatus.PARKED
    robot.current_coordinates = params["parking_spot"]
    return {"message":"Robot parked successfully"}
...
def handle_charge_command(robot: dict, params: dict) -> dict:
    ...
    if robot.status not in valid_states:
        raise ValueError("Can only charge robots that are COMPLETED or PENDING")
...
def handle_move_command(robot: dict, params: dict) -> dict:
    if "destination" not in params:
        raise ValueError("Move command requires destination parameter")
    if robot.status != RobotStatus.IN_PROGRESS:
        raise ValueError("Can only move robots that are IN_PROGRESS")
